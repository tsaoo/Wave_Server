...
int main(void){
	FILE* f_u = fopen(USERLIST_PATH,"rb+");
	fseek(f_u,0,SEEK_END);
	int c = ftell(f_u)/sizeof(struct User);
	struct User* userlist = (struct User*)(malloc(sizeof(struct User)*c));
	rewind(f_u);
	fread(userlist,sizeof(struct User),c,f_u);
	fclose(f_u);

	struct NewUser* newlist = (struct NewUser*)(malloc(sizeof(struct NewUser)*c));
	for(int i=0;i<c;++i){
		struct NewUser* newuser = (struct NewUser*)(malloc(sizeof(struct NewUser)*c));
		memcpy(&newuser->name,&userlist[i].name,100);
		memcpy(&newuser->pw,&userlist[i].pw,100);
		memcpy(&newuser->id,&userlist[i].id,sizeof(unsigned int));
		memcpy(newlist+i,newuser,sizeof(struct NewUser));
		free(newuser);
	}
	free(userlist);

	delete(USERLIST_PATH);
	f_u = fopen(USERLIST_PATH,"ab+");
	fwrite(newlist,sizeof(struct NewUser),c,f_u);
	fclose(f_u);
}